...
type SortKey = "code" | "name" | "packUnit" | "category" | "active";
...
export default function ProductsPage() {
  ...
  const [form, setForm] = useState({ code: "", name: "", packUnit: "", category: "", active: true });
  ...
  const filteredSorted = useMemo(() => {
    ...
    let rows =
      q === ""
        ? items
        : items.filter((r) =>
            [r.code, r.name, r.packUnit ?? "", r.category ?? ""]
  ...
  function openCreate() {
    setEditId(null);
    setForm({ code: "", name: "", packUnit: "", category: "", active: true });
    setError(null);
    setShow(true);
  }
  ...
  function exportCSV() {
    const header = ["Product Code", "Product Name", "Pack Unit", "Category", "Active"];
    const rows = filteredSorted.map((r) => [
      r.code, r.name, r.packUnit ?? "", r.category ?? "", r.active === false ? "No" : "Yes",
    ]);
    const csv =
      [header, ...rows]
        ...
        <table className="w-full border-collapse text-sm">
          <thead className="sticky top-0 z-10 bg-gray-50">
            <tr>
              ...
              <Th label="Product Code" active={sortKey === "code"} dir={sortDir} onClick={() => toggleSort("code")} width="160px" />
              <Th label="Product Name" active={sortKey === "name"} dir={sortDir} onClick={() => toggleSort("name")} />
            ...
            {loading ? (
              ...
              filteredSorted.map((r, idx) => (
                <tr key={r.id} className="odd:bg-white even:bg-gray-50">
                  ...
                  <td className="border px-2 py-2">{r.category ?? ""}</td>
                  <td className="border px-2 py-2">{r.active === false ? "No" : "Yes"}</td>
                </tr>
              ))
            ) : (
              <tr><td className="border px-3 py-3 text-gray-500" colSpan={7}>No products found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-[42rem] rounded bg-white p-6 shadow-lg">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-semibold">{editId == null ? "New Product" : "Edit Product"}</h2>
              <button className="text-gray-500 hover:text-black" onClick={() => setShow(false)} aria-label="Close">✕</button>
            </div>
            <form onSubmit={submit} className="grid grid-cols-2 gap-3">
              <Field label="Product Code"><input className="w-full rounded border p-2" value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} required /></Field>
              ...
              <Field label="Product Name" colSpan={2}><input className="w-full rounded border p-2" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} required /></Field>
              <Field label="Pack Unit"><input className="w-full rounded border p-2" value={form.packUnit} onChange={(e) => setForm((f) => ({ ...f, packUnit: e.target.value }))} /></Field>
              <Field label="Category"><input className="w-full rounded border p-2" value={form.category} onChange={(e) => setForm((f) => ({ ...f, category: e.target.value }))} /></Field>
